<template>
  <v-container>
    ...
    <div>
        <v-btn color="#2ECC71" @click="submit" style="margin-top: 10px;">อัพเดทรายวิชา</v-btn>
    </div>
  ...
  export default {
    ...
    //    alert(result.data.role)
       if(!result.data.login){
        ...
        await this.axios.post( 'http://localhost:3000/Subjects/getStudent' , { SubjectID : this.SubjectID , Sec: this.Sec }).then( res => {
            console.log(res)
    ...
    methods: {
      async submit(){
        let response = await this.axios.post( 'http://localhost:3000/Subjects/UpdateSubject' , { Professor : data.email , SubjectName : this.SubjectName , SubjectID : this.SubjectID , Students : this.Students , Sec: this.Sec })
        // console.log(re)
